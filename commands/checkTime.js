...
module.exports = {
    name: "time",
    execute: (msg, time ,Discord, help) => {
      if(help) {
        var memberN = msg.member.nickname;
        if (memberN === null) memberN = msg.author.username;
        msg.channel.send(new Discord.RichEmbed().setTitle('Witaj, ' + memberN).addField('Użycie:', 'yui!time <rola/użytkownik> <czas w dniach>').addField('Opis', 'Pokazuje listę osób którzy są dłużej niż ilość dni! Ew. sprawdza to dla jednej osoby'))
      } else {
        if(msg.member.hasPermission('MANAGE_ROLES') || utils.isOwner(msg.author.id)) {
        var memberN = msg.member.nickname;
        if (memberN === null) memberN = msg.author.username;
        let days = parseInt(time);
        let mention = require('./../mention.js')(msg), self, everyone, memberMentionedName, roles = msg.mentions.roles;
